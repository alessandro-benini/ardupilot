/// @file	AP_SerialBuffer.h
/// @brief	fifo (queue) buffer template class for serial communication
#pragma once

#include <stdint.h>

/// @class      AP_SerialBuffer
...
public:
	...
	int get_tail_position() { return tail; }

	int get_head_position() { return _head; }
...
template <class T, uint8_t SIZE>
AP_SerialBuffer<T, SIZE>::AP_SerialBuffer() :
_num_items(0), _head(0)
{
}

template <class T, uint8_t SIZE>
void AP_SerialBuffer<T, SIZE>::clear() {
	// clear the curve
	_num_items = 0;
	_head = 0;
}
...
template <class T, uint8_t SIZE>
void AP_SerialBuffer<T, SIZE>::push_back(const T &item)
{
...
template <class T, uint8_t SIZE>
bool AP_SerialBuffer<T, SIZE>::pop_front(T &ret)
{
	if (_num_items == 0) {
		// buffer is empty
		return false;
...
template <class T, uint8_t SIZE>
const T& AP_SerialBuffer<T, SIZE>::peek(uint8_t position) const
{
...
template <class T, uint8_t SIZE>
T& AP_SerialBuffer<T, SIZE>::peek_mutable(uint8_t position)
{
	uint8_t j = _head + position;
