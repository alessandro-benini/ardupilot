...
void Plane::generateVirtualWaypoints(const AP_Mission::Mission_Command& cmd)
{
    ...
    // Check if the next next waypoint is the landing waypoint
    ...
    if(wp.id == MAV_CMD_NAV_LAND && !vwp_status.vwp_generated)
    {
		...
		vwp_status.vwp_generated = true;

		// Retrieve wind estimation from EK2
		Vector3f wind;
		...
		// Next waypoint mission should be the last mission WP, since we get inito this procedure when we have 2 waypoints left
		// (1 mission waypoint and the landing waypoint).
		AP_Mission::Mission_Command last_mwp;
		mission.get_next_nav_cmd(cmd.index+(vwp_cfg.dist_lwp_idx-1), last_mwp);
		...
		Log_Write_VWP(vwp_cfg.first_vwp_idx++,vwp1_lat,vwp1_lng,vwp1_alt,1);
		...
		Log_Write_VWP(vwp_cfg.first_vwp_idx,land_wp_lat,land_wp_lng,land_wp_alt,1);
...
{
	generateVirtualWaypoints(cmd);
    set_next_WP(cmd.content.location);
}

void Plane::restoreMission()
{
