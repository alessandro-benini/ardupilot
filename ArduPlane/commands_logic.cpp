...
void Plane::calc_index_landing_waypoint(void)
{
	idx_landing_wp = mission.get_index_landing_WP();

	GCS_SEND_MSG("Index L_WP: %d",idx_landing_wp);
...
void Plane::calc_index_last_mission_waypoint(void)
{
	idx_last_mission_wp = mission.get_index_last_nav_WP();

	GCS_SEND_MSG("Index Last M_WP: %d",idx_last_mission_wp);
...
void Plane::calc_index_virtual_waypoints()
{
	vwp_cmd_idx = mission.get_index_for_VWP_generation(vwp_cfg.dist_lwp_idx);

	GCS_SEND_MSG("Index VWP: %d",vwp_cmd_idx);

	if(vwp_cmd_idx < 0)
	{
		GCS_SEND_MSG("LWP not found. VWP generation aborted.");
...
void Plane::generate_virtual_waypoints(const AP_Mission::Mission_Command& cmd)
{
	// variables storing the position info of the current NAV waypoint
    ...
    // This check is for debug only
    if(cmd.id == MAV_CMD_DO_CHANGE_SPEED && cmd.p1 == AUTOGENERATED_ITEM)
    	GCS_SEND_MSG("Reducing speed");

    // Check if the current cmd is where I should generate the virtual waypoints
    if(cmd.index == vwp_cmd_idx && vwp_error == VWP_NO_ERROR)
    {
		...
		mission.get_next_nav_cmd(idx_landing_wp, wp);
		...
		mission.get_next_nav_cmd(idx_last_mission_wp, last_mwp);
		...
		// --------------------------------------------------------------------------------
		// Difference of altitude between the landing waypoint and the last mission waypoint
		...
		if(g.vwp_enabled)
		{
			...
			mission.truncate(idx_landing_wp);
			...
			reduce_speed.p1 = AUTOGENERATED_ITEM;
...
void Plane::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    // Print (and save) information about the next waypoint
	...
	// Then, I check if I have to generate the virtual waypoints.
    // If the virtual waypoints have not been generated yet and there are no initialization errors
	if(vwp_status == VWP_NOT_GENERATED && vwp_error == VWP_NO_ERROR)
		// I check if it's time to generate them
		generate_virtual_waypoints(cmd);
}

void Plane::restore_mission()
{
